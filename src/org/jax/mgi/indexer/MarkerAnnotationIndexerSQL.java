...
public class MarkerAnnotationIndexerSQL extends Indexer {
	...
	public void index() throws Exception
	{
		...
		String annotToRefSQL = "select distinct annotation_key, reference_key from annotation_reference";

		logger.info(annotToRefSQL);
		HashMap<String, HashSet<String>> annotToRefs = makeHash(annotToRefSQL, "annotation_key", "reference_key");
		...
		while (!rs_overall.isAfterLast()) {
			...
			doc.addField(IndexConstants.BY_EVIDENCE_CODE, rs_overall.getString("evidence_code"));
			doc.addField(IndexConstants.BY_EVIDENCE_TERM, rs_overall.getString("evidence_code") + " - " + rs_overall.getString("evidence_term"));
		...
		logger.info("Done adding to solr; completed marker annotation index.");
		server.add(docs);
		server.commit();
	}
}
