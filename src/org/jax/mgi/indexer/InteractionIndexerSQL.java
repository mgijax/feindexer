...
public class InteractionIndexerSQL extends Indexer {
    ...
    public void index() throws Exception
    {
		...
		"select max(mi_key) as maxRegKey "
		+ "from marker_interaction");
            rs_tmp.next();
            
            int maxKey = rs_tmp.getInt("maxRegKey");

            System.out.println("max(mi_key): " + maxKey
		+ " Timing: " + ex.getTiming());
		...
		logger.info ("Processing mi keys " + startKey + " to "
		    + endKey);

		// gather our sets of optional properties for this chunk

	        HashMap<String, HashSet<String>> miToScore = makeHash(
		    propSQL.replace("<NAME>", "score")
			.replace("<START_KEY>", Integer.toString(startKey))
			.replace("<END_KEY>", Integer.toString(endKey)),
		    "mi_key", "value");

	        HashMap<String, HashSet<String>> miToSource = makeHash(
		    propSQL.replace("<NAME>", "data_source")
			.replace("<START_KEY>", Integer.toString(startKey))
			.replace("<END_KEY>", Integer.toString(endKey)),
		    "mi_key", "value");

	        HashMap<String, HashSet<String>> miToValidation = makeHash(
		    propSQL.replace("<NAME>", "validation")
			.replace("<START_KEY>", Integer.toString(startKey))
			.replace("<END_KEY>", Integer.toString(endKey)),
		    "mi_key", "value");

	        HashMap<String, HashSet<String>> miToTranscript = makeHash(
		    propSQL.replace("<NAME>", "mature_transcript")
			.replace("<START_KEY>", Integer.toString(startKey))
			.replace("<END_KEY>", Integer.toString(endKey)),
		    "mi_key", "value");

	        HashMap<String, HashSet<String>> miToNotes = makeHash(
		    propSQL.replace("<NAME>", "note")
			.replace("<START_KEY>", Integer.toString(startKey))
			.replace("<END_KEY>", Integer.toString(endKey)),
		    "mi_key", "value");
		...
		ResultSet rs = ex.executeProto (basicSQL
		    .replace("<START_KEY>", Integer.toString(startKey))
		    .replace("<END_KEY>", Integer.toString(endKey)) );
		...
		while (rs.next()) {
		    ...
		    String oMarkerSymbol = rs.getString(
			"organizerSymbol");
		    String pMarkerSymbol = rs.getString(
			"participantMarkerSymbol");
		    ...
		    doc.addField(IndexConstants.ORGANIZER_SYMBOL,
			oMarkerSymbol);
		    doc.addField(IndexConstants.PARTICIPANT_ID, pMarkerID);
		    doc.addField(IndexConstants.PARTICIPANT_SYMBOL,
			pMarkerSymbol);
			...
			doc.addField(IndexConstants.VALIDATION_SORTABLE,
			    validationToSort);
		    }
		    if (scoreToFilter != null) {
			doc.addField(IndexConstants.SCORE_FILTERABLE,
			    scoreToFilter);
		    }
		    if (scoreToSort != null) {
			doc.addField(IndexConstants.SCORE_SORTABLE,
			    scoreToSort);
		    }
